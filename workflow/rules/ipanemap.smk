...
def generate_ipanemap_config_from_snakeconf(
    ...
    generate_ipanemap_config(
        ...
        clustering_max_diameter_thres=config["ipanemap"]["clustering"][
            "max_diameter_threshold"
        ],
        clustering_max_avg_diameter_thres=config["ipanemap"]["clustering"][
...
def generate_ipanemap_config(
    ...
    ipanemap_conf["Clustering"]["MaxDiameterThreshold"] = str(
        clustering_max_diameter_thres
    )
    ipanemap_conf["Clustering"]["MaxAverageDiameterThreshold"] = str(
        clustering_max_avg_diameter_thres
    )
...
rule configure_ipanemap:
    output:
        cfg=expand(
            "results/{folder}/{rna_id}_pool_{pool_id}.cfg",
...
rule structure:
    ...
    output:
        expand(
            "results/{folder}/{rna_id}_pool_{pool_id}_{idx, \d+}.dbn",
