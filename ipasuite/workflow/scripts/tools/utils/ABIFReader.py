...
class ABIFReader:
    def __init__(self, fn):
        ...
        if self.type != "ABIF":
            self.close()
            raise SystemExit(f"error: No ABIF file {fn} - MagicStr : {self.type}")
    ...
    def getData(self, name, num=1):
        ...
        if not entry:
            raise SystemExit(
                "error: Entry '%s (%i)' not found in '%s'" % (name, num, self.filename)
            )
    ...
    def readNextChar(self):
        return self.primUnpack("c", 1).decode('ascii')
    ...
    def readNextDate(self):
        return datetime.date(
            self.readNextShort(), self.readNextByte(), self.readNextByte()
        )
